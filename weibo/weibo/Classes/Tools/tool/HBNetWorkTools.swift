...
class HBNetWorkTools: AFHTTPSessionManager {
    static let sharedTools:HBNetWorkTools={
        let tool: HBNetWorkTools = HBNetWorkTools()
...
//        tool.responseSerializer.acceptableContentTypes=//OC里面可以接收,swift里面没有返回值
       ...
       public func request(method:HTTPMethod, URLString: String, parameters: Any?,finish:@escaping (Any?,Error? )->())-> () {
